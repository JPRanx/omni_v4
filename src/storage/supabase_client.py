...
class SupabaseClient:
    ...
    def insert_vendor_payout(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Insert a vendor payout record."""
        result = self.client.table('vendor_payouts').insert(data).execute()
        return result.data[0] if result.data else {}

    def insert_vendor_payouts_batch(self, records: List[Dict[str, Any]]) -> int:
        """Insert multiple vendor payouts in batch."""
        if not records:
            return 0
